...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("touchmove", getColorFromWheel);
  document
    .getElementById("color-wheel")
    .addEventListener("mousemove", getColorFromWheel);
  document
    .getElementById("color-wheel")
    .addEventListener("click", merkerHandler);
  document
    .getElementById("color-wheel")
    .addEventListener("touchmove", merkerHandler);
  // document.getElementById('color-wheel').addEventListener('pointermove', merkerHandler);

  function merkerHandler() {
    document.getElementById("color-marker").style.background =
      hsv2hex(localColor);
  ...
  function getColorFromWheel(e) {
    ...
    if (
      Math.pow(markerX - wheel.offsetWidth / 2, 2) +
        Math.pow(markerY - wheel.offsetHeight / 2, 2) <=
      Math.pow(wheel.offsetHeight / 2, 2)
    ) {
...
export var nowPreset = 0;
export var colorsPresets = [];

export function setNowPreset(newPreset, newColorsPresets) {
  nowPreset = newPreset;
  var presets = document.getElementsByClassName("inner-color-item");
  for (let i = 0; i < presets.length; i++) {
    colorsPresets[i] = newColorsPresets[i];
    presets[i].style.background = hsv2hex(colorsPresets[i]);
  }
  presets[nowPreset].classList.add("selected-color-item");
  updateRangeState();
}
